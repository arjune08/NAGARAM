...
class Config:
    SECRET_KEY=os.environ.get('SECRET_KEY','nagaram-local-development-key')
    PERSISTENT_LOGIN_DAYS=3650
    PERSISTENT_LOGIN_LIFETIME=timedelta(days=PERSISTENT_LOGIN_DAYS)
    SESSION_COOKIE_NAME='nagaram_session';SESSION_COOKIE_HTTPONLY=True;SESSION_COOKIE_SECURE=True if os.environ.get('VERCEL') else False;SESSION_COOKIE_SAMESITE='None' if os.environ.get('VERCEL') else 'Lax';SESSION_COOKIE_DOMAIN=os.environ.get('SESSION_COOKIE_DOMAIN') or None;SESSION_COOKIE_PATH='/';SESSION_REFRESH_EACH_REQUEST=True;PERMANENT_SESSION_LIFETIME=PERSISTENT_LOGIN_LIFETIME
    ...
    MAX_CONTENT_LENGTH=16*1024*1024;RATELIMIT_STORAGE_URL='memory://'